...
func RecoverySequence(includeRestoration bool) []RecoveryStep {
	steps := []RecoveryStep{
	...
	if includeRestoration {
		steps = append(steps, RecoveryObserveRestore)
	}
	return steps
}
...
func RecoverySequenceForAction(action RecoveryAction, includeRestoration bool) []RecoveryStep {
	var steps []RecoveryStep
	switch action {
	case ActionValidate:
		steps = []RecoveryStep{RecoveryValidate}
	case ActionRebind:
		steps = []RecoveryStep{RecoveryObserveRoutes, RecoveryPark, RecoveryWithdraw, RecoveryQuiesce, RecoveryRebind, RecoveryValidate, RecoveryPublish, RecoveryResyncLeshy}
	case ActionRestartTransport:
		steps = []RecoveryStep{RecoveryObserveRoutes, RecoveryPark, RecoveryWithdraw, RecoveryQuiesce, RecoveryStartTransport, RecoveryValidate, RecoveryPublish, RecoveryResyncLeshy}
	case ActionRestartToad:
		return RecoverySequence(includeRestoration)
	default:
		return nil
	}
	if includeRestoration {
...
func RunRecovery(ctx context.Context, role *RoleRuntime, operation, epoch uint64, includeRestoration bool, work RecoveryWork) error {
	return RunRecoverySteps(ctx, role, operation, epoch, RecoverySequence(includeRestoration), work)
}
