...
Status: **PRIVILEGED HERMETIC ACCEPTANCE COMPLETE; INSTALLED-HOST CUTOVER PENDING**. The rebuilt orchestration gate uses the real core/config/Go-ownership contract and the 2026-09-25 privileged suite passes phases A-F, route parking, multi-Toad isolation and hermetic Xray while leaving host state unchanged. Real installed-host cutover and legacy retirement remain manual/operator decisions.
