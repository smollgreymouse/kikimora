...
# Trap-only helpers are intentionally referenced indirectly.
...
assert_no_other_vpn() {
    ...
    while IFS= read -r unit; do
        if [[ -n "$unit" && "$unit" != "$UNIT" ]]; then
            reasons+=("active Toad unit $unit")
        fi
...
package_diag() {
    ...
    sudo chown "$CALLER_UID:$CALLER_GID" "$DIAG/toad-journal.txt" "$DIAG/kernel-journal.txt" 2>/dev/null || true
    write_summary
    redact_diag
    compact_diag 0
...
if [[ ! "$HOLD_SECONDS" =~ ^[0-9]+$ ]] || (( HOLD_SECONDS < 10 || HOLD_SECONDS > 900 )); then
    fail "TOAD_SYSTEM_WIDE_HOLD_SECONDS must be 10..900"
fi
...
if ! ip -4 route show exact "$ENDPOINT_IP/32" | grep -q .; then
    ...
    if [[ -n "$UNDERLAY_GATEWAY" ]]; then
        endpoint_add+=(via "$UNDERLAY_GATEWAY")
    fi
    endpoint_add+=(dev "$UNDERLAY_DEV")
    if [[ -n "$UNDERLAY_SRC" ]]; then
        endpoint_add+=(src "$UNDERLAY_SRC")
    fi
...
if [[ ! "$GOOGLE_BYTES" =~ ^[0-9]+$ ]] || (( GOOGLE_BYTES < 10000 )); then
    fail "Google response was too short: ${GOOGLE_BYTES:-unknown} bytes"
fi
